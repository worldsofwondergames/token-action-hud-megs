...
import { prefixName, createHeroActor, deleteActor } from '../fixtures/test-data.mjs';
import {
    buildScene,
    captureModuleConfig,
    enableTah,
...
/**
 * End Turn belongs only to whoever is actually up. Offering it otherwise
 * invites a player to end someone else's turn.
 */
test('End Turn appears only on the actor\'s own turn', async ({ page }) => {
    ...
    try {
        ...
        firstId = await createHeroActor(page, prefixName('ET_A'), { dex: 8 });
        secondId = await createHeroActor(page, prefixName('ET_B'), { dex: 2 });
        scene = await buildScene(page, [firstId, secondId]);

        // 1. No combat at all.
        await selectToken(page, firstId);
        expect(await hudText(page)).not.toContain('End Turn');
        ...
        const currentActorId = await page.evaluate(async () => {
            ...
            for (const combatant of combat.combatants) {
                await combat.setInitiative(combatant.id, combatant.actor.system.attributes.dex.value);
            }
        ...
        expect(await hudText(page)).toContain('End Turn');

        await selectToken(page, otherActorId);
        const offTurn = await hudText(page);
        expect(offTurn).not.toContain('End Turn');
        ...
        expect(offTurn).toContain('Roll Initiative');
    } finally {
        await page.evaluate(async () => {
